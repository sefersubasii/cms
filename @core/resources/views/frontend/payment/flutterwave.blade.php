...
@section('site-title')
    {{get_static_option('site_'.$user_select_lang_slug.'_title')}} - {{get_static_option('site_'.$user_select_lang_slug.'_tag_line')}}
@endsection
@section('page-title')
...
{{--                <input type="hidden" name="phonenumber" value="090929992892" /> <!-- Replace the value with your customer phonenumber -->--}}
                <div class="btn-wrapper text-center">
                    <input type="submit" class="submit-btn" value="{{__('Pay '.$flutterwave_data['amount'].get_charge_currency('flutterwave').' Now')}}"  />
                </div>
            </form>
        </div>
    </div>
